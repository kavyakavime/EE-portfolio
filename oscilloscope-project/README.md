...
## Results (Updated to Match Measured Data)
...
#### Measured cutoff frequency (from your plot)
...
- **Measured cutoff (approx):** **~ 400 Hz** (interpolated between your ~330 Hz and ~520 Hz points)

> Note: This measured cutoff differs from the theoretical 159 Hz, which is expected given the non-ideal measurement chain and scaling issues described below.
...
## Error Analysis (Deeper + More Accurate for Your Setup)
...
**Fix (recommended):**
- Convert ADC counts using **5.0 V** (or measure actual AVcc with a multimeter and use that value).
- Or use the Nano’s **internal reference** / external reference (advanced improvement).
...
**Fix (recommended):**
- Keep sweeps below ~800–900 Hz for clean results OR increase sampling rate and use more careful timing.
...
**Fix (recommended):**
- Use timer interrupts + binary packet streaming (instead of println) for more uniform sampling.
...
**Fix (recommended):**
- Add a simple RC smoothing filter after the DAC (or use an external DAC).
...
**Fix (recommended):**
- Measure R and C with a meter (if available) and compute theoretical cutoff using measured values.
