...
### Frequency-Domain Response (FFT Analysis)

The frequency-domain plot shows the FFT magnitude of the sampled signal, confirming the presence of dominant frequency components generated by the ESP32 function generator. A strong peak appears near **~875 Hz**, where the FFT magnitude reaches approximately **100**, corresponding to the primary input sine-wave frequency used during testing.

A secondary peak is observed near **~220 Hz** with a magnitude of approximately **45**. This lower-amplitude component is attributed to harmonic distortion and non-idealities in the signal generation and sampling chain, including DAC quantization (8-bit ESP32 DAC), stepped waveform generation, and ADC sampling artifacts.

Smaller spurious peaks across the spectrum are also visible and are primarily caused by quantization noise, serial timing jitter, and spectral leakage due to the finite sampling window. Overall, the FFT confirms correct frequency generation while highlighting expected non-ideal behavior in a low-cost, software-defined measurement system.
